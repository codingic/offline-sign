...
/// 钱会打进没人控制的地址，且不报错。
///
/// # 领域说明：为什么是 P2WPKH 而不是 P2PKH
///
/// 同一个公钥能派生两类地址，本服务只支持 **P2WPKH**：
/// 它是 2017 年 SegWit 以来的主流类型（见证数据享 75% 权重折扣、手续费更低，
/// 且签名不计入 txid，修掉了交易延展性），主流钱包的默认接收地址都是 `bc1q…`。
/// P2PKH（`1…`）是 2017 年前的遗留类型，本签名器会明确拒绝它。
///
/// 为什么不能两类都支持：它们的 sighash 算法**互不相通**（BIP143 含金额 vs
/// 传统算法不含），同时支持就等于要求调用方时刻记住「地址类型要跟签名算法配套」。
/// 只留一种，配错的代价从「广播失败」前移成「签名时立刻报错」。
/// 详见 `sign/btc.rs` 的模块文档。
///
/// # 领域说明：为什么固定主网
///
